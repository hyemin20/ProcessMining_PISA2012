...
### Set file path
df <- read.csv("D:/HYEM'S/GraduatedSchool/PROJECTS/MyProjects/ProcessMining_PISA2012/datasets/3_Outliers_3_FullC_with_ln_scoring_gender_time_cleaning.csv", 
...
cfAirq5 <- cforest(weight~ time_A_total+time_b1+time_b2+time_B_total+time_explore+time_start+time_answer+time_not_answer+time_solving+time_total+avg_time_btw_events+length+events_num, data = df, 
...
score_ratio_RF <- randomForest(ratio ~ time_a+time_b1+time_b2+time_start+time_answer+time_irrelevant+time_explore_relevant+time_solving+time_total+avg_time_btw_events+length+events_num, data = df, 
...
[8] "time_solving"          "time_total"            ""           ""       "" "time_ticket_explore"   "time_explore_total"   
[15] "time_avg_explore"      "time_avg_btw_events"   "events_num"            "sequence"              "length"
...
df_train <- df[idx,]
df_test <- df[-idx,] 
rfAirq5 <- randomForest(ratio ~ time_A_total+time_b1+time_b2+time_start+time_answer+time_not_answer+time_solving+time_total+avg_time_btw_events+length+events_num, data = df_train, 
                        importance = TRUE, keep.forest = TRUE, keep.inbag = TRUE)
## train data의 rmse
...
sqrt(mean((df_test$ratio-mt)^2))

## test data의 rmse_2
rmse(df_test$ratio, mt)
sd(df_test$ratio-mt)
