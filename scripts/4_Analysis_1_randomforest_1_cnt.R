...
### Set file path
df <- read.csv("D:/HYEM'S/GraduatedSchool/PROJECTS/MyProjects/ProcessMining_PISA2012/datasets/3_OutliersNormalization_3_FullC_7.0_tem.csv", 
...
### test set, train set
df$rankings <- as.factor(df$cnt_code)
...
# random forest model
rf_imp <- ranger(score ~ time_A_total+time_b1+time_b2+time_start+time_answer+time_not_answer+avg_time_btw_events, data = df_top, importance = 'impurity')
rf_perm <- ranger(score ~ time_A_total+time_b1+time_b2+time_start+time_answer+time_not_answer+avg_time_btw_events, data = df_top, importance = 'permutation')
